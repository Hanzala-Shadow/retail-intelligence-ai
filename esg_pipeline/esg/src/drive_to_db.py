...
def tracker_filename(note: str | None) -> str | None:
    """The PDF filename recorded in a tracker row's ``notes``.

    ``notes`` is the only place the tracker names the file a report row refers
    to, and 12 of the 681 rows append a human comment after it::

        AAP-ADVANCE AUTO PARTS INC-2017-2018.pdf (filename spans 2017-2018)

    Those 12 are exactly the reports whose filename carries two years, which is
    also the case where guessing the year from the filename goes wrong: the
    tracker assigns 2018, a naive parse takes 2017. Stripping the comment and
    trusting ``report_year`` is what makes the mapping exact rather than
    inferred.
    """
    text = (note or "").strip()
    if not text:
        return None
    match = re.match(r"^(?P<name>.*?\.pdf)\s*(?:\(.*\))?$", text, re.IGNORECASE)
    return (match.group("name") if match else text).strip() or None


def build_report_year_by_filename(tracker_rows: list[dict], anomalies: list[str]) -> dict[str, int]:
    """Map lowercased PDF filename -> report year, from the tracker.

    Lowercased because the corpus carries both ``.pdf`` and ``.PDF`` spellings
    and a case-sensitive match drops the three documents that differ.
    """
    mapping: dict[str, int] = {}
    for row in tracker_rows:
        name = tracker_filename(row.get("notes"))
        year = parse_int(row.get("report_year"))
        if not name or year is None:
            continue
        key = name.lower()
        if key in mapping and mapping[key] != year:
            anomalies.append(
                f"tracker names {name} for both {mapping[key]} and {year}; keeping {mapping[key]}"
            )
            continue
        mapping.setdefault(key, year)
    return mapping
...
def build_documents(
    ...
    report_year_by_filename: dict[str, int] | None = None,
) -> list[dict]:
    documents: dict[tuple[str, str], dict] = {}
    year_by_filename = report_year_by_filename or {}

    def report_year_for(filepath: str) -> int | None:
        """Report year for a document, from the tracker rather than the name."""
        name = Path(filepath).name.lower()
        if not name:
            return None
        year = year_by_filename.get(name)
        if year is None and year_by_filename:
            anomalies.append(f"{name}: no tracker row names this file, so it has no report year")
        return year
    ...
    for row in parse_rows:
        ...
        documents[key] = {
            ...
            **lineage,
            ...
            "report_year": report_year_for(source_pdf),
        }
        ...
        for pdf_file in pdf_files:
            ...
            documents.setdefault(
                key,
                {
                    ...
                    "report_year": report_year_for(display_path(pdf_file)),
                },
            )
...
def build_plan(args) -> LoadPlan:
    ...
    report_year_by_filename = build_report_year_by_filename(tracker_rows, plan.anomalies)
    plan.documents = build_documents(
        parse_rows, plan.companies, local_pdfs, plan.anomalies, identity_lookup,
        report_year_by_filename,
    )
...
def load_db_dependencies():
    ...
    try:
        ...
        from common.models import Company as _Company
        from common.models import SustainabilityReport as _SustainabilityReport
        from common.models import LogicalSource as _LogicalSource
        from common.models import SourceVersion as _SourceVersion
        from common.models import FileAlias as _FileAlias
        from common.models import ExtractionArtifact as _ExtractionArtifact
        from common.models import SourceApproval as _SourceApproval
        from common.models import Document as _Document
        from common.models import Section as _Section
        from common.models import Chunk as _Chunk
...
def apply_plan(plan: LoadPlan) -> dict[str, int]:
    ...
    with SessionLocal() as session:
        ...
        # Keyed by (ticker, year) so documents can resolve their report below.
        # Reports are flushed before documents are touched, so report_id exists
        # by then; without that ordering the FK would be unassignable.
        report_map: dict[tuple[str, int], SustainabilityReport] = {}
        for row in plan.reports:
            ...
            report_map[(row["ticker"], row["year"])] = report
        ...
        for row in plan.documents:
            ...
            if report_year is not None:
                report = report_map.get((row["ticker"], report_year))
                if report is not None:
                    document.report_id = report.report_id
                else:
                    counts["documents_without_report"] += 1
                    plan.anomalies.append(
                        f"{row['ticker']} {row['pdf_stem']}: tracker says {report_year} "
                        f"but no report row exists for that company-year"
                    )
            else:
                counts["documents_without_report"] += 1
            session.flush()
