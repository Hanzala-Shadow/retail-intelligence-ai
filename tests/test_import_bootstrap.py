...
def _runs_work_at_import(path: Path) -> bool:
    ...
    # A top-level `if` that raises is an environment gate, and it is checked
    # before the __main__ guard because having a guard does not make one safe:
    # db_loader.py and chunks_bulk_loader.py both carry a guard AND do
    #
    #     if not DB_URL:
    #         raise EnvironmentError("DB_URL or DATABASE_URL is not set.")
    #
    # then build a SQLAlchemy engine at module level. Importing those to prove
    # they parse would need a live database. The `config` tell below cannot see
    # them: the name they read is DB_URL, not config.
    #
    # Deliberately narrower than "any module-level raise" -- that matches 33
    # more modules, mostly `try: import x / except: raise`, and guts the check.
    ...
    # A __main__ guard is otherwise the author's statement that importing this
    # module does nothing. The eleven that do pipeline work at import are
    # exactly the ones that lack it.
