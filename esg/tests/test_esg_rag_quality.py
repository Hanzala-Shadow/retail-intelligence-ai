...
class ESGRAGQualityTests(unittest.TestCase):
    def test_source_aligned_chunk_trims_within_overlap_when_retokenized_size_expands(self) -> None:
        class BoundaryEncoder:
            @staticmethod
            def encode(text: str) -> list[int]:
                return list(range(601 if len(text) == 500 else 599))

        text = "a" * 700
        chunk_text, start, end, token_count = esg_chunker.bounded_source_aligned_chunk_text(
            text,
            list(range(len(text))),
            0,
            500,
        ...
        self.assertEqual(len(chunk_text), 499)
        self.assertEqual((start, end), (0, 499))
        self.assertEqual(token_count, 599)
