...
def enriched():
    if not os.path.exists(ENRICHED_INDEX):
        pytest.skip("run esg/src/esg_p1_enrichment.py first")
...
def test_embedding_text_derives_from_source_chunk(enriched):
    ...
    The derivation is both steps the writer runs: normalize the chunk text,
    then prepend the context header. Asserting on normalize_for_embedding
    alone silently stopped covering the stored file the day the header was
    added -- the copy on disk has carried it since.
    """
    _, rows = enriched
    checked = 0
    for row in rows:
        ...
        with open(os.path.join(REPO_ROOT, rel), encoding="utf-8") as fh:
            stored = fh.read()
        recomputed = p1.build_embedding_text(row, p1.normalize_for_embedding(source))
