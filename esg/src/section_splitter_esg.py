...
HEADING_PATTERNS: list[tuple[str, str]] = [
    ...
    # `social compliance` deliberately absent -- see supply_chain_ethics above.
    ("social", r"^social$|^society$|\b(corporate\s+social\s+responsibility|social\s+impact|social\s+sustainability|social\s+responsibility)\b"),
...
def _local_navigation_ribbon_indexes(
    ...
    Geometry-based furniture removal in the bridge remains the primary rule.
    This is a fallback for legacy or synthetic flattened text that reaches the
    splitter without that geometry. A title alone is never enough: an
    occurrence must have the local shape of a navigation ribbon on repeated
    pages. A later real body heading with the same words remains eligible.
    """
    ...
    for indexes in fuzzy_groups:
        if (
            len({page_positions[index][0] for index in indexes})
            >= NAVIGATION_CHROME_MIN_OCCURRENCES
        ):
            rejected.update(indexes)
    return rejected
...
@dataclass(frozen=True)
class SectionTask:
    """One isolated parsed document, safe to section in a worker process."""
...
def _run_section_task(task: SectionTask) -> dict:
    """Plan and write one document; the parent remains the sole index writer."""
    ticker = task.txt_file.parent.name.upper()
    try:
        source_fingerprint = fingerprint_source_file(task.txt_file)
        text = task.txt_file.read_text(encoding="utf-8", errors="replace")
        page_spans = read_page_map(task.txt_file)
        heading_offsets = read_heading_map(task.txt_file) or None
        ...
        if task.resume and not task.force and validation.complete:
            return {"status": "skipped", "ticker": ticker, "pdf_stem": task.txt_file.stem}

        rows = process_text_file(
            task.txt_file,
            task.output_root,
            text=text,
...
def run(
    ...
    print(f"Section workers: {workers}")
    ...
    for txt_file in txt_files:
        ...
        document_rows = {
            key: row
            for key, row in index_rows.items()
            if key[0] == ticker_name and key[1] == stem
        }
        document_duplicates = {
            key for key in duplicate_keys if key[0] == ticker_name and key[1] == stem
        }
        tasks.append(
            SectionTask(
                txt_file=txt_file,
                output_root=output_root,
                existing_rows=document_rows,
                duplicate_keys=document_duplicates,
                resume=resume,
                force=force,
            )
        )
    ...
    if workers == 1:
        results = map(_run_section_task, tasks)
        executor = None
    else:
        executor = ProcessPoolExecutor(max_workers=workers)
        results = executor.map(_run_section_task, tasks)

    try:
        for result in results:
            ticker_name = result["ticker"]
            stem = result["pdf_stem"]
            if result["status"] == "skipped":
                summary["skipped_complete"] += 1
                print(f"  {ticker_name} {stem}: skipped (complete)")
                continue
            if result["status"] == "failed":
                summary["failed"] += 1
                print(f"  {ticker_name} {stem}: failed ({result['error']})")
                continue

            file_rows = result["rows"]
            _replace_pdf_rows(index_rows, ticker_name, stem, file_rows)
            duplicate_keys = {
                key for key in duplicate_keys if not (key[0] == ticker_name and key[1] == stem)
            }
            ...
            print(f"  {ticker_name} {stem}: {len(file_rows)} section(s)")
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Section independent documents in N worker processes while the "
            "parent process writes the shared section index (default: 1)."
        ),
    )
