...
Authenticates through common/drive_auth.py, the shared OAuth desktop flow
(client_secret.json + token.json). This module used to carry its own copy of
that flow -- as did drive_downloader.py and drive_uploader.py -- which is why
splitting the pipelines forced the extraction: two of the three copies are ESG
and one is 10-K. If you have already authenticated for any of them, this works
without a new browser prompt.

Usage:
    python esg/src/count_drive_pdfs.py
    python esg/src/count_drive_pdfs.py --folder-id 1AbCdEfGhIjKlMnOp
    python esg/src/count_drive_pdfs.py --folder-name "Sustainability Reports"
    python esg/src/count_drive_pdfs.py --max-depth 2
    python esg/src/count_drive_pdfs.py --out reports/drive_pdf_counts.csv
"""
...
SCOPES = drive_auth.READONLY_SCOPES
