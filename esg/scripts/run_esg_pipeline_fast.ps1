...
param(
    ...
    # Resolved from common/config.py once $Paths is loaded -- param defaults are
...
try {
    ...
    if ($runIntake) {
        ...
        $arguments.Add("esg/src/esg_intake_catalog.py")
    ...
    if ($runParse) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/pdf_parser.py")
    ...
    if ($runRemediate) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/pipeline_ocr_remediation_stage.py")
    ...
    if ($runSection) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/section_splitter_esg.py")
        $arguments.Add("--resume")
        $arguments.Add("--input")
        $arguments.Add($Paths.ESG_TEXT_DIR)
        $arguments.Add("--out")
    ...
    if ($runChunk) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/esg_chunker.py")
        $arguments.Add("--resume")
        $arguments.Add("--input")
        $arguments.Add($Paths.ESG_SECTIONS_DIR)
        $arguments.Add("--out")
    ...
    if ($runLayout) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/esg_layout_qa.py")
        $arguments.Add("--resume")
        $arguments.Add("--parse-index")
        $arguments.Add($Paths.ESG_PARSE_INDEX_CSV)
        $arguments.Add("--out")
    ...
    if ($runQa) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/src/esg_pipeline_qa.py")
        $arguments.Add("--out")
    ...
    if ($runEnrich) {
        ...
        Invoke-PythonStage -Name "enrich" -Arguments @(
            "esg/src/esg_p1_enrichment.py"
        )
    }

    if ($runManifest) {
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/scripts/build_esg_vector_manifest.py")
        $arguments.Add("--chunks-index")
        $arguments.Add($Paths.ESG_CHUNKS_INDEX_CSV)
        ...
        $arguments.Add($Paths.ESG_CHUNKS_INDEX_ENRICHED_CSV)
        $arguments.Add("--out")
    ...
    if ($runValidate) {
        ...
        $arguments = [System.Collections.Generic.List[string]]::new()
        $arguments.Add("esg/scripts/validate_esg_provenance.py")
        $arguments.Add("--parse-index")
        $arguments.Add($Paths.ESG_PARSE_INDEX_CSV)
        $arguments.Add("--sections-index")
        $arguments.Add($Paths.ESG_SECTIONS_INDEX_CSV)
    ...
    if ($runTests) {
        ...
        Invoke-PythonStage -Name "tests" -Arguments @(
            # Both suites: esg/tests is this pipeline's, tests/ is the
            # cross-cutting one that polices the config split itself.
            "-m", "pytest", "esg/tests", "tests", "-q"
        )
    }
