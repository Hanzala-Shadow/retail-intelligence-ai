"""Put the ESG pipeline's directories on ``sys.path``, for ``esg/scripts/esg_database_tiers/``.
...
Running ``python esg/scripts/esg_database_tiers/qa_tier1_invariants.py`` puts
``esg/scripts/esg_database_tiers/`` on ``sys.path`` automatically, which is why
this module needs no path hack of its own. It is one directory deeper than
``esg/scripts/_bootstrap.py``, hence the extra ``.parent``. See
