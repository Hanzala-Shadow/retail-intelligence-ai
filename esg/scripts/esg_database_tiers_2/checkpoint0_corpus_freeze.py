...
Q1 and Q2 need the parsed text and page maps under the run root's
interim/esg_text/ (ESG_RUN_ROOT in esg/config.py; --esg-text-root to override)
...
def render(results: list[CheckResult], snapshot: dict) -> None:
    ...
    print(f"run root         : {snapshot['esg_run_root']}")
...
def main() -> int:
    ...
    # The parsed text lives with the build the database was loaded from, not in
    # data/02_interim -- see ESG_RUN_ROOT in esg/config.py.
    parser.add_argument("--esg-text-root", type=Path, default=config.ESG_RUN_TEXT_DIR)
    ...
    snapshot = {
        ...
        "esg_run_root": str(config.ESG_RUN_ROOT),
