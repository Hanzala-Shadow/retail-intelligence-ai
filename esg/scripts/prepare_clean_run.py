...
def _rel(path) -> str:
    ...
    ``ROOT`` is monkeypatched to a temporary tree by the tests and points at
    the live repo otherwise, so the trees this script clears must stay
    relative. They are still derived from config, so the layout lives in one
    place.
    """
...
REF_REL = _rel(config.REFERENCE_DIR)
...
OUTPUT_TREES = {
    _rel(config.ESG_TEXT_DIR): "parsed text",
    _rel(config.ESG_SECTIONS_DIR): "section text",
    _rel(config.ESG_CHUNKS_DIR): "chunk text (citation evidence -- rebuilt by the reparse)",
    _rel(config.EMBEDDING_DIR): "embedding text copies",
}
...
def main(argv=None) -> int:
    ...
    if not args.no_backup:
        (backup / REF_REL).mkdir(parents=True, exist_ok=True)
        for name, path in to_clear:
            shutil.copy2(path, backup / REF_REL / name)
