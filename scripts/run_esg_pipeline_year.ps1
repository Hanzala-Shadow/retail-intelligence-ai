...
#
# Only the parse stage is year-aware (src/pdf_parser.py --years). Every later
...
try {
    ...
    Invoke-PythonStage -Name "manifest" -Arguments @(
        "scripts/build_esg_vector_manifest.py",
    ...
    Invoke-PythonStage -Name "enrich" -Arguments @("src/esg_p1_enrichment.py")
    ...
    if (-not $SkipTests) {
        Invoke-PythonStage -Name "tests" -Arguments @("-m", "pytest", "tests", "-q")
    }
