...
LAYOUT_AUDIT_VERSION = "layout_v2"
...
def verify_written_manifest(path: Path, chunks: list[dict]) -> dict[str, int]:
...
def build_manifest(
    ...
    rows = sorted(
        (manifest_row(chunk, registry, layout_audit) for chunk in chunks),
        key=lambda row: row["chunk_id"],
    )
    atomic_write_csv(Path(out), rows)
    verify_written_manifest(Path(out), chunks)
    return rows
...
def main() -> None:
    ...
    verification = verify_written_manifest(Path(args.out), read_csv(Path(args.chunks_index)))
