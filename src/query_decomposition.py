...
class SourceResolver:
    def __init__(
        ...
        sections_by_source: dict[tuple[str, int, str, str], set[str]] | None = None,
    ):
        self.records = tuple(records)
        self._sections_by_source = {
            key: frozenset(values)
            for key, values in (sections_by_source or {}).items()
        }

    @classmethod
    def from_connection(cls, conn: Any) -> "SourceResolver":
        sql = """
          SELECT DISTINCT ticker, filing_year, doc_type, accession_number, section_code
          FROM public.rag_eligible_10k_chunks
          ORDER BY ticker, filing_year, doc_type, accession_number, section_code
        """
        with conn.cursor() as cursor:
            cursor.execute(sql)
            rows = cursor.fetchall()
        records: dict[tuple[str, int, str, str], FilingRecord] = {}
        sections: dict[tuple[str, int, str, str], set[str]] = {}
        for ticker, year, doc_type, accession, section in rows:
            key = (str(ticker), int(year), str(doc_type), str(accession))
            records.setdefault(key, FilingRecord(*key))
            sections.setdefault(key, set()).add(str(section))
        return cls(records.values(), sections)

    @staticmethod
    def _source_key(record: FilingRecord) -> tuple[str, int, str, str]:
        return (
            record.ticker,
            record.filing_year,
            record.doc_type,
    ...
    def sections_for(self, record: FilingRecord) -> frozenset[str]:
        """Return eligible sections when the live catalog supplied them."""
        return self._sections_by_source.get(self._source_key(record), frozenset())
