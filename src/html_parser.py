...
#     def _extract_tables(self, soup: BeautifulSoup) -> list[TableRef]:
#         tables = []
...
#             csv_path = self.table_output_dir / f"table_{i}.csv"
...
#             t0 = time.time()
#             try:
#                 doc = parser.parse(f, company=company, doc_type="10-K")
...
# Same "suspiciously thin parse" threshold pdf_parser.py uses to decide
# whether an .htm 10-K parsed cleanly (MIN_HTML_CHARS there). Kept in sync
# here so both parsers agree on what counts as a failed/garbled extraction.
MIN_TEXT_CHARS = 2000
    ...
    for company, files in companies.items():
        ...
        for f in targets:
            ...
            try:
                ...
                if doc.status == "ok" and doc.char_count > 0:
                    # Same output filename schema as pdf_parser.py:
                    # {company}__{doc_type}__{stem}.txt
